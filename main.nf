...
t1_for_test_n4
    .map{it -> if(!params.run_n4_bias_correction){it}}
    .mix(t1_n4)
...
process Lesion_On_Anat{
    ...
    output:
    set sid, "${sid}__mask_lesion_10.nii.gz" into lesion_mask_on_anat
    set sid, "${sid}__lesion_10.nii.gz" into lesion_for_dicom
